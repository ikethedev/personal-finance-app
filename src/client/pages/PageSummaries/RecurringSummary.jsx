...
const RecurringSummary = () => {
    ...
    const recurring = startData.transactions.filter(item => item.recurring === true)
...
const formatDate = (dateString) => {
 ...
 const getCategorySums = () => {
    ...
    return recurring.reduce((totals, item) => {
 ...
 console.log(paidTotal != 0)
 console.log(dueSoonTotal === 0)
 console.log(upcomingTotal === 0)
    ...
    return (
        ...
        <div className={styles.category}>
            <div className={`${styles.category__container} ${styles.paid}`}>
                <p>Paid Bills</p>
                {paidTotal === 0 ? <p>no recent bills paid</p> : `${Math.abs(paidTotal)}` }
            </div>
            <div className={`${styles.category__container} ${styles.soon}`}>
                <p>Due Soon</p>
                {dueSoonTotal === 0 ? <p>no upcoming bills</p> : `${Math.abs(dueSoonTotal)}` }
            </div>
            <div className={`${styles.category__container} ${styles.upcoming}`}>
                <p>Upcoming Bills</p>
                {upcomingTotal === 0 ? <p>no bills for the rest of the month</p> : `${Math.abs(upcomingTotal)}` }
            </div>
        </div>
    </div>
    )
